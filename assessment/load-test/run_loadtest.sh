...
# NOVIDADE: Descobrir onde o script está guardado
...
# CORREÇÃO: Usamos "$SCRIPT_DIR/reset_db.sql" para ele saber onde está o ficheiro
docker exec -i nopcommerce_mssql_server /opt/mssql-tools18/bin/sqlcmd -S localhost -U SA -P "nopCommerce_db_password" -C -d nopcommerce_db < "$SCRIPT_DIR/reset_db.sql"

if [ $? -eq 0 ]; then
  echo -e "${GREEN}✓ Rules applied successfully!${NC}"
else
  echo -e "${RED}An error occurred in the SQL script. Trying fallback path...${NC}"
  docker exec -i nopcommerce_mssql_server /opt/mssql-tools/bin/sqlcmd -S localhost -U SA -P "nopCommerce_db_password" -C -d nopcommerce_db < "$SCRIPT_DIR/reset_db.sql"
fi

echo ""

# Step 2: Clear nopCommerce Cache
...
# CORREÇÃO: Apontar o k6 para o ficheiro na mesma pasta do script
k6 run "$SCRIPT_DIR/loadtest.js"
